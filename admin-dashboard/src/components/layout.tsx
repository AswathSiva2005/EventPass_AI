import { Building2, CalendarPlus, LayoutDashboard, LogOut, Menu, Moon, ScanLine, Sun, UserCog, Users, X } from "lucide-react";
...
const navigation = [
  ...
  ,{ to: "/admin-accounts", label: "Admin accounts", icon: UserCog, superAdminOnly: true }
];
...
export const Layout = () => {
  ...
  const sidebar = (
    <>
      ...
      <nav className="mt-4 grid gap-1 px-3">
        {navigation.filter(item=>!item.superAdminOnly||user?.role==="super_admin").map(({to,label,icon:Icon})=><NavLink key={to} to={to} end={to==="/"} onClick={()=>setOpen(false)} className={({isActive})=>`focus-ring flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-bold transition ${isActive?"bg-lime-300 text-ink-950":"text-slate-500 hover:bg-slate-100 hover:text-ink-950 dark:text-slate-300 dark:hover:bg-white/6 dark:hover:text-white"}`}><Icon size={18}/>{label}</NavLink>)}
      </nav>
  ...
  return <div className="min-h-screen bg-slate-200/80 lg:grid lg:grid-cols-[248px_1fr] dark:bg-slate-950">
    ...
    <div className="min-w-0 lg:col-start-2">
      <header className="sticky top-0 z-30 flex h-18 items-center justify-between border-b border-slate-300 bg-slate-200/90 px-4 backdrop-blur-xl sm:px-7 dark:border-white/8 dark:bg-slate-950/90">
