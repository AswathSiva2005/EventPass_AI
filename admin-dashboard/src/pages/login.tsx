import { ArrowRight, Eye, EyeOff, LockKeyhole, ScanLine } from "lucide-react";
...
export const LoginPage=()=>{
  ...
  return <main className="grid min-h-screen lg:grid-cols-[1.05fr_.95fr]">
    ...
    <section className="grid place-items-center p-6"><form onSubmit={(e)=>void handleSubmit(submit)(e)} className="w-full max-w-md"><span className="grid size-12 place-items-center rounded-2xl bg-lime-300 text-ink-950"><LockKeyhole/></span><h2 className="mt-7 font-display text-4xl font-bold tracking-[-.05em]">Admin sign in</h2><p className="mt-3 text-sm leading-6 text-slate-500">Use your verified EventPass administrator account.</p>
      ...
      <label className="mt-5 block text-sm font-bold">Password<div className="relative"><input type={showPassword?"text":"password"} autoComplete="current-password" className="focus-ring mt-2 h-12 w-full rounded-xl border border-slate-200 bg-white px-4 pr-12 dark:border-white/10 dark:bg-white/5" {...register("password",{required:"Password is required"})}/><button type="button" onClick={()=>setShowPassword(value=>!value)} className="absolute top-2 right-0 grid h-12 w-12 place-items-center text-slate-500 hover:text-slate-900 dark:hover:text-white" aria-label={showPassword?"Hide password":"Show password"}>{showPassword?<EyeOff size={18}/>:<Eye size={18}/>}</button></div></label>{errors.password&&<p className="mt-1 text-xs text-rose-500">{errors.password.message}</p>}
